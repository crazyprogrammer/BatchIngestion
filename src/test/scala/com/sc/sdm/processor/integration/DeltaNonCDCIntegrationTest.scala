...
class DeltaNonCDCIntegrationTest extends FlatSpec with Matchers with SdmSharedSparkContext {
  "DeltaNonCdc New DataSriTest" should "createSriOpen_NonOpen" in {
      ...
      val sriParams: SriParams = IntegrationTestDataHelpers.createSriParams(id, partition, partition, eodMarker, SourceType.BATCH_DELIMITED, base, vTypesAuditColSchema, sriAuditColSchema, systemAuditColSchema, tableName.split("_")(2), "start_date", "yyyy-MM-dd")
        .copy(recordTypeBatch = "Y", timestampCol = "start_date", timestampColFormat = "yyyy-MM-dd")
      ...
      assert(sqlSriOpen2.take(1).map(_.getLong(0)).head == 4)
      val sqlSriNonOpen2: DataFrame = spark.sql("select count(*) from " + sriParams.sriNonOpenSchema + "." + tableName + " where nds='2017-01-02'")
      assert(sqlSriNonOpen2.take(1).map(_.getLong(0)).head == 7)


      //Assert previous day as well
      assert(spark.sql(s"select count(*) from ${sriParams.sriOpenSchema}." + tableName + " where ods='2017-01-01'").take(1).map(_.getLong(0)).head == 3)
      assert(spark.sql(s"select count(*) from ${sriParams.sriNonOpenSchema}." + tableName + " where nds='2017-01-01'").take(1).map(_.getLong(0)).head == 4)
      assert(spark.sql(s"select rowcount from ${sriParams.opsSchema}.gps_all_rowcounts where rcds='2017-01-01' and step_name='Duplicates'").take(1).map(_.getLong(0)).head == 2)
