...
class RecordTypeTransactionTest extends FunSuite with SdmSharedSparkContext with Checkers with NonCDCTestFixtures {
  ...
  def getAllAssertions(verifyTypesList: List[DelimitedTableRowWithRecordType], previousSriList: List[DelimitedTableRowWithRecordType], mayBeDeletables: List[DelimitedTableRowWithRecordType]) = {
    ...
    //val deletables:List[DelimitedTableRowWithRecordType] = List()

    val duplicates = verifyTypesList.last.copy(rowId = s"${System.nanoTime()}_${UUID.randomUUID().toString}")

    val verifyTypesWithDeletes = (verifyTypesList ++ deletables ++ List(duplicates))
    ...
    val SriOutput(sriOpen, sriNonOpen, duplicatesOpt, _, bRecordsOpt) = processor.processSri(tableName, dataCols, List("drReason"), todaysVerifyTypesDF, previousSriDFDUMMY, metaInfo, TableType.TXN, RunType.INCREMENTAL, januaryFirst)(spark)
    ...
    //val updatedVerifyTypesSet = verifyTypesWithDeletes.filterNot(_.recordType == "B").toSet
    val updatedVerifyTypesSet = verifyTypesWithDeletes.toSet

    val latestPerKeyInVTypes = verifyTypesList
    ...
    val deletedPrimaryKeys = deletables.map(vType => s"${vType.drReason}")
    val rowsWithDeletablesPK = latestPerKeyInVTypes.filter(vType => deletedPrimaryKeys.contains(s"${vType.drReason}"))
    ...
    val sriOpenCountCheck = sriOpen.count() === latestPerKeyInVTypes.size - deletables.size

    //SRI NON_OPEN = prevSRI Data which is updated through today's VTypes + Changes during the day in Vtypes + D records - bRecords
    val sriNonOpenCountCheck = sriNonOpen.count() === oldUpdatesDuringTheDay.size + deletables.size - duplicatesRowIds.size

    //Actual record checks
    val sriOpenRowIdCheck = sriOpenRowIds === latestPerKeyInVTypes.map(_.rowId).diff(rowsWithDeletablesPK.map(_.rowId))
    ...
    val sriNonOpenRowIdCheck = sriNonOpenRowIds === (oldUpdatesDuringTheDay.map(_.rowId) ++ rowsWithDeletablesPK.map(_.rowId)).diff(duplicatesRowIds)
